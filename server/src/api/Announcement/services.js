...
export default {
    ...
    isAvailable: async (announcement, user) => {
        const permissions = mg.cache.roles[user.roleId].permissions;

        const roles = getPermission(permissions, ['announcement', 'read']);
        const classes = getPermission(permissions, ['class', 'multiple']);
